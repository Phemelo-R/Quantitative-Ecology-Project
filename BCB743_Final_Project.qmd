...
author: "Shane Ngwenya, Phemelo Rutlokoane, Kezia Samuels"
date: "17 June 2025"
format:
  html:
    toc: true
...
## 1. Biological Data acquisition and processing (Shane Ngwenya)
...
### Pre-processing the raw adder data

This one will wait until Shane goes back to the lab. Until then, we move...
...
## 2. Environmental data sourcing and extraction (Phemelo Rutlokoane)
...
## 3. Data analysis and visualisation (Kezia Samuels)

### Load the data

```{r}
# Environmental data
num_env <- read_csv("Num_env_var.csv")
View(num_env)

# Species abundance data 
adder_abd <- read_csv("Adder_Spe_ABD_Clean.csv")
View(adder_abd)
```
...
```{r}
env <- num_env %>% 
  select(-c(1, 2)) # Remove first two columns (lat and long)
View(env)
...
Calculate the matrix with p-values

```{r}
rcorr(as.matrix(env)) # Calculate correlation matrix with p-values
```
...
corrplot(env_corr, 
         ...
         tl.srt = 90,               # Rotate labels 90 degrees
...
Calculate the correlation matrix for the land cover variables

```{r}
land_corr <- round(cor(env_landcover), 2)
land_corr
```
...
```{r}
rcorr(as.matrix(env_landcover)) # Calculate correlation matrix with p-values
```

Plot the pearson correlation matrix for the landcover variables
...
Check for multicollinearity using VIF for the land cover variables

```{r}
# Check for multicollinearity using VIF
vif_result_land <- vifstep(env_landcover, th = 10)  # threshold of 10 is common, can also use 5

# See which variables are retained
vif_result_land
```

Although the Forested land variable seems to have collinearity issues from the VIF results, I did not remove this variable from the dataset.
...
```{r}
Env_var_clean <- read_csv("Env_var_clean.csv")
View(Env_var_clean)
...
```{r}
# Get site scores
site_scores <- data.frame(scores(cca_result, scaling = 2, display = "sites"))
site_scores$site <- rownames(site_scores)
site_scores$biome <- env$biome[match(site_scores$site, rownames(env))]
...
arrow_scale <- 3  # Adjust this multiplier as needed
biplot_scores_scaled <- biplot_scores
biplot_scores_scaled$CCA1 <- biplot_scores$CCA1 * arrow_scale
biplot_scores_scaled$CCA2 <- biplot_scores$CCA2 * arrow_scale
...
```{r}
env_landcover <- rownames_to_column(env_landcover, var = "site")
```

Group the Forested land, Shrubland, Grassland, Waterbodies, Wetlands, Barren Land columns into a column called "natural" and sum the proportions

```{r}
env_landcover$natural <- rowSums(env_landcover[c("Forested land", "Shrubland", "Grassland", "Waterbodies", "Wetlands", "Barren Land")]) 
```
...
env_landcover <- env_landcover%>% 
  select(-c(`Forested land`, `Shrubland`, `Grassland`, `Waterbodies`, `Wetlands`, `Barren Land`))
```

Group the Cultivated, Built-up, Mines & Quarries columns into a column called "natural" and sum the proportions

```{r}
env_landcover$transformed <- rowSums(env_landcover[c("Cultivated", "Built-up", "Mines & Quarries")])
```
...
```{r}
env_landcover <- env_landcover%>% 
  select(-c(`Cultivated`, `Built-up`, `Mines & Quarries`))
```

Create a grouping variable for dominant landcover

```{r}
env_landcover <- env_landcover %>%
  mutate(dominant_landcover = ifelse(natural > transformed, "Natural", "Transformed"))
```

Run nMDS

Prepare the data for nMDS (only numeric columns)

```{r}
landcover_data <- env_landcover %>%
  select(natural, transformed)  # Only the numeric landcover columns
View(landcover_data)
```

Use "canberra" as it is the most appropriate distance measure for proportion data

```{r}
# Run nMDS with Canberra distance
nmds_result <- metaMDS(landcover_data, distance = "canberra", k = 2)
```
...
Add the grouping variable and site names

```{r}
nmds_scores$dominant_landcover <- env_landcover$dominant_landcover
nmds_scores$site <- env_landcover$site
```

Create the plot

Using ggplot2 with envfit()

```{r}
# Extract species scores for all species
species_scores <- as.data.frame(scores(species_fit, display = "vectors"))
species_scores$species <- rownames(species_scores)
species_scores$pval <- species_fit$vectors$pvals

# Create the ggplot
ggplot(nmds_scores, aes(x = NMDS1, y = NMDS2, color = dominant_landcover)) +
  geom_point(size = 2, alpha = 0.8) +
  geom_segment(data = species_scores, 
               aes(x = 0, y = 0, xend = NMDS1 * 1.5, yend = NMDS2 * 1.5), 
               arrow = arrow(length = unit(0.5, "cm")), 
               color = "red", inherit.aes = FALSE, size = 0.8) +
  geom_text_repel(data = species_scores, 
                  aes(x = NMDS1 * 1.8, y = NMDS2 * 1.8, label = species), 
                  color = "red", inherit.aes = FALSE, 
                  size = 3, fontface = "italic",
                  max.overlaps = Inf,
                  box.padding = 0.3,
                  point.padding = 0.2) +
  scale_color_manual(values = c("Natural" = "#2E8B57", "Transformed" = "#CD853F")) +
  labs(title = "nMDS Ordination with Species Vectors",
       subtitle = "Canberra Distance",
       x = "NMDS1", 
       y = "NMDS2",
       color = "Dominant Landcover") +
  theme_minimal() +
